...
* There are missing values in `prcp`, `snow`, `snwd` and especially in `tmin` and `tmax`, in which missing values take about `r ny_noaa %>% pull(tmin) %>% is.na() %>% sum()/nrow(ny_noaa)*100 `% and `r  ny_noaa %>% pull(tmax) %>% is.na() %>% sum()/nrow(ny_noaa)*100`% of total records respectively.
