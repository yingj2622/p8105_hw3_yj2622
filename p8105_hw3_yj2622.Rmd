...
```{r}
accel_df %>% 
  ggplot(aes(x = minute, y = n_activ, color = day)) +
  scale_x_continuous(
  ...
  geom_point(alpha = .4) +
  geom_line(data = accel_df %>% filter(week == 1)) +
  geom_line(data = accel_df %>% filter(week == 2)) +
  geom_line(data = accel_df %>% filter(week == 3)) +
  geom_line(data = accel_df %>% filter(week == 4)) +
  geom_line(data = accel_df %>% filter(week == 5))
  
```

Based on the plot above, it can be seen that in general, the man is most active at night (about 20 to 22 o'clock) and least active after 22 o'clock until the next day morning (about 6 o'clock). We can also see that he is very active in the noon, about 11 to 12 o'clock, on Sunday. 