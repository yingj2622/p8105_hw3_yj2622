...
We can see that:
  ...
  - There are NAs in records of weather stations,especially in `tmin`
    and `tmax`.
...
Next, I make plots of `year` vs average `tmax` oberved in differnt
...
Below are interpretions of the plots:
  ...
  - There are outliers of the average maximum temperature in both
    ...
    temperature recordered by one station.
